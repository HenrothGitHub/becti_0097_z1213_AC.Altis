...
player addAction ["<t color='#3333bb'>Network : Reconnect </t>", "player setVariable ['CTI_Net',CTI_P_SideID,true];", "", -101, false, true, "", "!CTI_P_PreBuilding &&!((player getVariable 'CTI_Net')==CTI_P_SideID) "];
...
player addAction ["<t color='#006400'>Start Intrusion</t>", "[(vehicle player),player] call AN_E_Intrusion_Start", [], -200, false, true, "", "((vehicle player) getVariable ['CTI_Net',-10]) >= 0 && !((vehicle player) getVariable ['CTI_Net',-11] == player getVariable ['CTI_Net',-11]) && (speed (vehicle player)) <1 && (speed (vehicle player)) >-1 && !((vehicle player) == player )&& alive player && ((CTI_P_SideLogic getVariable 'cti_upgrades') select CTI_UPGRADE_EXPI) > 0 && ! AN_E_INTRU"];
//
