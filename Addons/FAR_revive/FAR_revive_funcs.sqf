...
FAR_Player_Actions =
{
	0 spawn {
		...
		_trackdeath={
			...
			sleep random(1);
			FAR_OBJ_ACTIONS=FAR_OBJ_ACTIONS-[_this]-[objNull];
		};
		while {!CTI_GameOver} do { // to rework its ugly like that
			_o = (getPos player) nearEntities ["Man",100];
			{
				_c=_x;
				if ({_x == _c} count FAR_OBJ_ACTIONS ==0 ) then {
					_c addAction ["<t color=""#C90000"">" + "Revive" + "</t>", "Addons\FAR_revive\FAR_handleAction.sqf", ["action_revive"], 10, true, true, "", "call FAR_Check_Revive"];
					sleep random(1);
					FAR_OBJ_ACTIONS=FAR_OBJ_ACTIONS+[_c]-[objNull];
					(_c) spawn _trackdeath;
				};
			} forEach _o;
			sleep 5;
		};
	...
	if (alive player && player isKindOf "Man") then
	{
		...
		//player addAction ["<t color=""#C90000"">" + "Revive" + "</t>", "Addons\FAR_revive\FAR_handleAction.sqf", ["action_revive"], 10, true, true, "", "call FAR_Check_Revive"];
		//player addAction ["<t color=""#C90000"">" + "Stabilize" + "</t>", "Addons\FAR_revive\FAR_handleAction.sqf", ["action_stabilize"], 10, true, true, "", "call FAR_Check_Stabilize"];
...
FAR_HandleDamage_EH =
{
	...
	if (alive _unit &&_amountOfDamage < 1 && _amountOfDamage >= 0.75 && _isUnconscious == 0) then
	{
...
FAR_Player_Unconscious =
{
  ...
  _unit allowDamage false;
	...
	waitUntil {(_unit getVariable "FAR_isUnconscious" == 1)};
...
FAR_Check_Revive =
{
	private [ "_isTargetUnconscious", "_isDragged"];
	_return = false;
	_isPlayerUnconscious = 0;
	// Unit that will excute the action
	if (!(isnil {_this getVariable "FAR_isUnconscious"})) then {
		_isPlayerUnconscious = _this getVariable "FAR_isUnconscious";
	} else {
		_isPlayerUnconscious = 0;
	};
	//diag_log [_target,_this];
	//_isPlayerUnconscious = _caller getVariable "FAR_isUnconscious";
	_isMedic = getNumber (configfile >> "CfgVehicles" >> typeOf _this >> "attendant");
	//_target = cursorTarget;
	//diag_log [_isPlayerUnconscious,FAR_isDragging,isNil "_target",!alive _target,(!isPlayer _target && !FAR_Debugging), !(side _target == side _this) ];
	// Make sure player is alive and target is an injured unit
	if( !alive _this || _isPlayerUnconscious == 1 || FAR_isDragging || isNil "_target" || !alive _target || (!isPlayer _target && !FAR_Debugging) || (isPlayer _this && (_this distance _target)>2) || !(side group _target == side group _this) ) exitWith
	{
		_return
	};
	//diag_log "touched";
...
FAR_Check_Suicide =
{
	...
	if (! isPlayer _this) exitWith{_return};
...
FAR_Check_Dragging =
{
	...
	if( !alive player || _isPlayerUnconscious == 1 || FAR_isDragging || isNil "_target" || !alive _target || (!isPlayer _target && !FAR_Debugging) || (_target distance player) > 2 || !(side group _target == side group player)) exitWith
	{
		_return;
	};
