	...
	while {!CTI_GameOver} do {
		{
			...
			_ci=0;
			{
				...
				if (count (missionNamespace getVariable (format ["CTI_BASES_%1_NEIGH",_side]) select _ci) !=3 ) then {
						...
						_off=if !(MADE_FOR_STRATIS) then {500} else {0};
						{
							[["CLIENT",_side], "Client_Connect",[[_base select 0,_base select 1,0],getPos _x,"ColorBrown",_off]] call CTI_CO_FNC_NetSend;
...
while {!CTI_GameOver} do {
	{
		...
		{
			_cbase=_x;
			_objects = ([_cbase select 0,_cbase select 1,0] nearEntities ["AllVehicles", (CTI_BASE_AREA_RANGE)]) unitsBelowHeight 40;
			if ( ({_ci == _x} count _detected) !=1 && (_enemy countSide _objects) > 0 ) then {
				...
				{
					[["CLIENT",_enemy], "Client_Connect",[[_cbase select 0,_cbase select 1,0],getPos _x,((_side) call CTI_CO_FNC_GetSideColoration),_off]] call CTI_CO_FNC_NetSend;
				} forEach _ctown;
			};
			_ci = _ci +1;
		} forEach _bases;
