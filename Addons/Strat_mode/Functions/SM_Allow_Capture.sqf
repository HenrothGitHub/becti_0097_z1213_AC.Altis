...
while  {!CTI_GameOver} do {
	...
	{
		_t=_x;
		_forces=if ((_t getVariable "cti_town_occupation_active") || (_t getVariable "cti_town_resistance_active") ) then {true} else {false}; //forces present
		_neigh=if ({((_x) getVariable "cti_town_sideID") == _id_s} count (_t getVariable "CTI_Neigh") == 0 && count ((_side) call CTI_CO_FNC_GetSideTowns)>0) then {false} else {true}; // neighbours check
		...
		if !(!_prevent && _neigh && (_forces || _priority && ! _ours)) then {
			...
			[["CLIENT",_x],"SM_message",format ["Town %1 is now Inactive",(_t getVariable "cti_town_name")],true] call CTI_CO_FNC_NetSend ;
			_n_ac_s = _n_ac_s - [_t];
		};
			true
	} count _ac_s;
...

	{
		...
		_no_limit=if (count _n_ac_s <CTI_SM_MAX_ACTIVE) then {true} else {false}; // we can still active
		_prevent=if (_t ==_stop_s) then {true} else {false}; //prevent check
