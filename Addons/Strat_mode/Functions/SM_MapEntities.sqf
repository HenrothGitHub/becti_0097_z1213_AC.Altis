...

{
	...
	if (true) then {
		...
		if ((_side_id == player getvariable ["CTI_Net",-1]|| _object in HUD_T_OBJ || _object in (_sl getVariable "CTI_HUD_SHARED")) && (player getVariable ["CTI_Net",-10])>= 0) then {
			...
			_text="";
			if !(_object isKindOf "Man" && (_side_id == player getvariable ["CTI_Net",-1])) then {
