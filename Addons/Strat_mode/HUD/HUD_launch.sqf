...
HUD_UpdateVehicle={
	...
	if (vehicle	player == player  ) then{
		...
		if (missionNamespace getVariable "CTI_EW_ANET" == 1 && ! isNil {(vehicle player) getVariable "CTI_Net"} && ! isNil {(vehicle player) getVariable "AN_iNet"}) then {
			_co="";
			if (((vehicle player) getVariable "AN_iNet") == (vehicle player) getVariable "CTI_Net") then {_co=format ["<t color='#00ff00'>%1</t>",(((vehicle player) getVariable "CTI_Net") call CTI_CO_FNC_GetSideFromID)]} else {_co=format ["<t color='#ffff00'>%1</t>",(((vehicle player) getVariable "CTI_Net") call CTI_CO_FNC_GetSideFromID)]};
