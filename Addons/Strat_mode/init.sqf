	...
	with missionNamespace do {
		...
		CTI_PVF_Server_Base_Connect= {
			...
			_off=if !(MADE_FOR_STRATIS) then {500} else {0};
			{
				...
				{
					[["CLIENT",_req_p], "Client_Connect",[[(_bases select _ci) select 0,(_bases select _ci) select 1,0],getPos _x,"ColorBrown",_off]] call CTI_CO_FNC_NetSend;
				} forEach _ctown;
				_ci=_ci+1;
			} forEach _conn;
		};
		...
		CTI_PVF_Server_Base_Detec ={
...

			{
				...
				[["CLIENT",_req_p], "Client_Base_Zone",[_enemy,_x,[_cbase select 0,_cbase select 1,0]]] call CTI_CO_FNC_NetSend;
					...
					{
						[["CLIENT",_req_p], "Client_Connect",[[_cbase select 0,_cbase select 1,0],getPos _x,((_enemy) call CTI_CO_FNC_GetSideColoration),_off]] call CTI_CO_FNC_NetSend;
					} forEach _ctown;
					_ci=_ci+1;
			} forEach _d_base;
		};
