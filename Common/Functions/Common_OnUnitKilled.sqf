...
if (_killer isKindOf "UAV" && ! isNull((uavControl _killer) select 0)  ) then {_killer = ((uavControl _killer) select 0)};
...
if (!isnil "GUER_ZEUS" && side group _killer == resistance ) then {

	if (_killer in (curatorEditableObjects GUER_ZEUS)) then {
		_z_points=((_type_killed) call Z_GUER_Prices)/3;
		["SERVER", "Server_Addpoints_Zeus",_z_points] call CTI_CO_FNC_NetSend;
	};
};