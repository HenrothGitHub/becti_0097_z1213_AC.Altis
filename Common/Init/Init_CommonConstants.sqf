...
CTI_TOWNS_OCCUPATION_GROUPS_RATIO = 0.015; //--- Determine how many groups may spawn (scales with town value)
...
with missionNamespace do {
	...
	if ( CTI_DEBUG ) then
	{
		CTI_ECONOMY_STARTUP_FUNDS_EAST_COMMANDER = 250000;
	}
	else
	{
	...
	if ( CTI_DEBUG ) then
	{
		CTI_ECONOMY_STARTUP_FUNDS_WEST_COMMANDER = 250000;
	}
	else
	{
		if (isNil 'CTI_ECONOMY_STARTUP_FUNDS_WEST_COMMANDER') then {CTI_ECONOMY_STARTUP_FUNDS_WEST_COMMANDER = 90000};
	};
	...
	if (isNil 'CTI_PLAYERS_GROUPSIZE') then {CTI_PLAYERS_GROUPSIZE = 12}; //Done
	...
	if (isnil "CTI_SM_PATROLS") then {CTI_SM_PATROLS = 1};
